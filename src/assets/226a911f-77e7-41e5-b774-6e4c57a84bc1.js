...
function OptSlider({ title = "BetMaker" }) {
  const app = window.useApp();
...
Promise.resolve(window.__spriteReady).then(() => {
  [["optA", OptA], ["optB", OptB], ["optC", OptC], ["optD", OptD], ["optE", OptE], ["optF", OptF], ["optG", OptG], ["optH", OptH], ["optI", OptI], ["optDirect", OptDirect], ["optSlider", OptSlider], ["optLucky", () => <OptSlider title="Lucky Slip" />]].forEach(([id, C]) => {
    const el = document.getElementById(id);
    if (el) ReactDOM.createRoot(el).render(<Frame><C /></Frame>);
  });
});
